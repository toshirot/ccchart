//ccchart plugin http://ngw.jp/~tato/wp/?p=389
   ...
   window.ccchart.m.crossPointer = {
      ...
      setLineColor: function (op){
        //線の色
        this.crossPointer.lineColor = op.lineColor || 'rgba(255, 255, 120, 0.7)';
        //横線の色
        this.crossPointer.xColor = op.xColor || this.crossPointer.lineColor || 'rgba(255, 255, 120, 0.7)';
        //縦線の色
        this.crossPointer.yColor = op.yColor || this.crossPointer.lineColor || 'rgba(255, 255, 120, 0.7)';
      },
      setLineWidth: function (op){
        //線の幅
        this.crossPointer.lineWidth = op.lineWidth || 1;
        //横線の幅
        this.crossPointer.xWidth = op.xWidth || this.crossPointer.lineWidth || 1;
        //縦線の幅
        this.crossPointer.yWidth = op.yWidth || this.crossPointer.lineWidth || 1;
      },
      //移動する縦ライン
      crossPointer: function (op){
        ...
        this.crossPointer.currXY=[];//現在のマウス位置
        ...
        function drawXLine(x, y){
          ...
          //横線の色指定
          ctx.strokeStyle = that.crossPointer.xColor;
          //横線の幅
          ctx.lineWidth = that.crossPointer.xWidth;
          //線を描画する
          ctx.stroke();
          ctx.restore();
        ...
        function drawYLine(x, y){
          ...
          //縦線の色指定
          ctx.strokeStyle = that.crossPointer.yColor;
          //縦線の幅
          ctx.lineWidth = that.crossPointer.yWidth;
          //線を描画する
          ctx.stroke();
          ctx.restore();
        }
